...
type TaxRateResponse struct {
	...
	StateOrProvinceID *uint     `json:"stateOrProvinceId,omitempty"`
...
func (s *TaxService) UpdateTaxClass(ctx context.Context, id uint, name string) (*TaxClassResponse, error) {
	var tc TaxClassResponse
	err := s.db.QueryRowContext(ctx, `
...
func (s *TaxService) GetTaxRates(ctx context.Context, page, pageSize int) ([]TaxRateResponse, int64, error) {
	...
	for rows.Next() {
		...
		if stateID.Valid {
			v := uint(stateID.Int64)
			tr.StateOrProvinceID = &v
		}
...
func (s *TaxService) CreateTaxRate(ctx context.Context, taxClassID uint, countryID string, stateOrProvinceID *uint, rate float64, zipCode string) (*TaxRateResponse, error) {
	var tr TaxRateResponse
	var stateID sql.NullInt64
	err := s.db.QueryRowContext(ctx, `
...
func (s *TaxService) UpdateTaxRate(ctx context.Context, id uint, taxClassID *uint, countryID *string, stateOrProvinceID *uint, rate *float64, zipCode *string) (*TaxRateResponse, error) {
...
func (s *TaxService) GetTaxRate(ctx context.Context, id uint) (*TaxRateResponse, error) {
	var tr TaxRateResponse
	var stateID sql.NullInt64
	err := s.db.QueryRowContext(ctx, `
		SELECT id, tax_class_id, country_id, state_or_province_id, rate, COALESCE(zip_code,''), created_at, updated_at
